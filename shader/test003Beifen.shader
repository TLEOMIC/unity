    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                half4 lit = _Color * mainTexCol;
                fixed4 col2 =  lerp(lerp(lit,shaderMainTexCol * _ShadeColor,lightIntensity) * beishu ,lerp(mainTexCol2 * _Color,shaderMainTexCol2 * _ShadeColor2,lightIntensity) * beishu2,rate);
                ...
                switch(_NeedLightSwitch){
                    ...
                    half3 indirectLighting = lerp(toonedGI, ShadeSH9(half4(worldNormal, 1)), 1);
                    indirectLighting = lerp(indirectLighting, max(EPS_COL, max(indirectLighting.x, max(indirectLighting.y, indirectLighting.z))), 1); // color atten
